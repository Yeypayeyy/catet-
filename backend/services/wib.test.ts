...
describe("batas WIB dengan awal bulan", () => {
  it("tanpa key, 17 Sep dengan mulai 28 = periode September", () => {
  ...
  it("periode lampau dihitung penuh", () => {
    assert.equal(batasBulan("2026-02", new Date("2026-09-17T05:00:00Z"), 28).hari, 31);
    assert.equal(batasBulan("2026-02", new Date("2026-09-17T05:00:00Z")).hari, 28);
  });

  it("tahun dengan mulai 28", () => {
    assert.equal(batasTahun(2026, 28).awal.toISOString(), "2025-12-27T17:00:00.000Z");
    assert.equal(batasTahun(2026, 28).akhir.toISOString(), "2026-12-27T17:00:00.000Z");
  });
});
